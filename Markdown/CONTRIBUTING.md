...
## Repository Settings Checklist (GitHub UI)

Apply these in repo settings so automation can run safely:

- Enable repository auto-merge.
- Keep squash merge enabled (release automation merges PRs with `SQUASH`).
- Add branch protection/rulesets for `dev`, `prev`, and `main`:
  - Require pull requests before merging.
  - Require at least 1 approval.
  - Require conversation resolution before merge.
  - Restrict direct pushes (only maintainers/bots that must operate there).
- Require status checks:
  - `Project Checks / checks`
  - `Changeset Check / changeset`
  - On `dev` also require `Validate PR title / Validate PR title`
- Add a tag ruleset for `v*`:
  - Restrict tag create/update/delete to release maintainers.
  - Allow the GitHub Actions app/bot if your stable release workflow needs to create the stable `vX.Y.Z` tag.
